...
;
;
function nspace(n){var result;(result="");while((0<n)){(result=(result+" "));(n=(n-1))};return(result)};
function prettyprint(list,acc,indent){var str,i,escape,seppos,first,sep,length;if(!(acc)){(acc=array());prettyprint(list,acc,4);return(array_join(acc,""))}else if((list[0]==="num")){array_push(acc,list[1]);return(1)}else if((list[0]==="str")){(escape={"\n":"\\n","'":"\\'","\t":"\\t","\\":"\\\\","\r":"\\r"});(str=list[1]);array_push(acc,"'");(i=0);while((i<len(str))){array_push(acc,(escape[str[i]]||str[i]));(i=(i+1))};array_push(acc,"'");return(len(str))}else if((typeof(list)==="string")){array_push(acc,list);return(len(list))};array_push(acc,"[");(length=1);(seppos=array());(first=true);(i=0);while((i<len(list))){if(!(first)){array_push(seppos,len(acc));array_push(acc,"")};(length=((length+1)+prettyprint(list[i],acc,(indent+4))));(first=false);(i=(i+1))};if(((110-indent)<length)){(sep=strjoin("\n",nspace(indent)))}else {(sep=" ")};(i=0);while((i<len(seppos))){put(acc,seppos[i],sep);(i=(i+1))};array_push(acc,"]");return(length)};
;
;
;
function compile_stdin(){print("/* Code compiled from yolan */");print("load(\"stdlib.js\");");(expr=getnext());while(!((expr===false))){print(compile(expr));(expr=getnext())}};
